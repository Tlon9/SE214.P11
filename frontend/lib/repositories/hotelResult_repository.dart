...
class HotelResultRepository {
  ...
  Future<List<Hotel>> fetchHotels({
    ...
    List<Hotel> hotels = await dataProvider.fetchHotelResults(
        searchInfo: searchInfo, offset: offset, limit: limit);
    if (hotels.isEmpty) {
    ...
    if (filterOption != null && filterOption.isNotEmpty) {
      hotels = hotels
          .where((hotel) =>
              hotel.price! >= int.parse(filterOption['minPrice']!) &&
              hotel.price! <= int.parse(filterOption['maxPrice']!) &&
              filterOption['selectedStars'].contains(hotel.rating!))
          .toList();
    }
